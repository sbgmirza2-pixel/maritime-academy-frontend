...
import { faCubes, faClock, faDollarSign, faCheckCircle } from '@fortawesome/free-solid-svg-icons';

const CargoManagement = ({ data, onBook }) => {
  const title = data?.title ?? data?.name ?? "Cargo Management";
  const description = data?.description ?? data?.details ?? "Efficient cargo planning, tracking, documentation and logistics management for international maritime transportation.";
  const price = data?.price ?? 1800;
  const duration = data?.duration_days ?? 6;
  const features = data?.features || [
    "Cargo Flow Planning & Stowage",
