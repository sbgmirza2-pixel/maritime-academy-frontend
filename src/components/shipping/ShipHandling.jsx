...
import { faShip, faClock, faDollarSign, faCheckCircle } from '@fortawesome/free-solid-svg-icons';

const ShipHandling = ({ data, onBook }) => {
  const title = data?.title ?? data?.name ?? "Ship Handling";
  const description = data?.description ?? data?.details ?? "Professional ship handling services including vessel operations, docking assistance and maritime navigation support.";
  const price = data?.price ?? 1500;
  const duration = data?.duration_days ?? 5;
  const features = data?.features || [
    "Vessel Operations Control",
    "Docking and Undocking Assistance",
